...
/*   Updated: 2025/08/20 14:47:53 by yukravch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_waiting_for_threads(t_general *main)
{
	...
	{
		if (pthread_join(main->philo[i].threads_id, NULL) == ERROR)
			return (ERROR);
		i++;
	}
	return (SUCCESS);
...
int	ft_init_philos(t_general *main)
{
	...
	while (i < main->nb_of_philo)
	{
		...
		if (pthread_create(&main->philo[i].threads_id,
					NULL, ft_routine, &main->philo[i]) == ERROR)
			return (ERROR);
		i++;
	}
